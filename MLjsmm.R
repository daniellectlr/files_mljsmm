jsmm.mcmc <- function(loglikelihood = loglik, data = data, n.iter = n.iter, n.adapt.iter = iter, n.thin = 1, rotate = TRUE){
  ...
  RES = as.vector(B%*%XTheta) - as.vector(Thetah) ###############
  li2 <- -(1/2) * RES %*% iVII %*% RES # o primeiro res nao deveria ser transposto? Testei e dá o mesmo valor
  ...
  for (i in 1:(n.iter + n.adapt.iter)){
    ...
    for (ii in 1:n.thin){
      ...
      for (ll in 1: nl){ # update thetah for each landscape
        ...
        for (l in 1:np){ # update thetah for each parameter
          ...
          for(k in 1:ns){ # update thetah for each species
            ...
            nTHETAH <- Thetah[k, ,ll] #Thetah_ls[k, ]
          ...
          for (k in 1:ns){
            ...
            RES = as.vector(B%*%XTheta) - as.vector(N2THETAH) ###############
      ...
      # XThetah <- as.vector(aperm(Thetah, c(1,3,2))) # transform to vector by line
      XThetah <- as.vector(Thetah) # transform to vector by column
      ...
      Vt <- (Vt + t(Vt))/2 ### necessary??
      musTHE = Vt %*% (iXSI %*% (Q %*% z) + t(B) %*% iVII %*% XThetah)
      # set.seed(42)
      XTheta <- as.vector(rmvnorm(1, mean = musTHE, sigma = Vt))
      # THE = matrix(XTHE, ncol = np * nc, byrow=TRUE) ################ adicionei ", byrow=TRUE)" em 21/11/17
      ...
      # Z <- matrix(z, ncol = np*nc, byrow = TRUE)
      # M <- TT %*% Z
      ...
      # RES <- t(matrix(RES, ncol = nl * ns, byrow = TRUE))
      RES <- matrix(RES, ncol = np)
      A <- t(RES) %*% solve(diag(nl)%x%diag(ns)) %*% RES
      PSIA_V <- PSI_V + A
      PSIA_V <- (PSIA_V + t(PSIA_V))/2
      ##set.seed(42)
      # V <- riwish((nu_V + ns), PSIA_V) # Inverse Wishart Matrix Distribution; np*nc x np*nc matrix
      ...
      if(includePhylogeny){
        ...
        RES <- XThetah - as.vector(B %*% XTheta) 
        li2 <- -(1/2) * RES %*% iVII %*% RES ### RES tranposto ???
